...
def printNode(node):
    ...
    print('Number of moves:', len(node[9:]))
    print('Moves:', node[9:])
    print('------')
    nodeNumber += 1
...
if __name__ == '__main__':
    ...
    startNode = list(map(int,input("\nEnter the starting state : ").strip().split()))[:9]
    finalNode = list(map(int,input("\nEnter the final state : ").strip().split()))[:9]
    ...
    print('The above one is the answer using BFS method|')
    print('------')
